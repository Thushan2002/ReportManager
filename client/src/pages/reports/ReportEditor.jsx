...
export const ReportEditor = () => {
  ...
  const totalPlannedHours =
    report.tasks?.reduce((sum, t) => sum + (Number(t.plannedHours) || 0), 0) ||
    0;
  const totalSpentHours =
    report.tasks?.reduce((sum, t) => sum + (Number(t.spentHours) || 0), 0) || 0;
  const avgCompletion =
    report.tasks?.length > 0
      ? Math.round(
          report.tasks.reduce(
            (sum, t) => sum + (Number(t.actualPercent) || 0),
            0,
          ) / report.tasks.length,
        )
      : 0;
    ...
    <div className="workspace-page report-editor">
      <header className="page-header">
        <div>
          ...
          <p>
            Every report follows the identical, standardized team format for
            clear visibility.
          </p>
        </div>
        <span
      ...
      {status === "Needs Correction" && (
        <div className="review-note">
          <div
            ...
            <FiAlertCircle />{" "}
            <strong>Manager Feedback Requiring Correction:</strong>
          </div>
          <p>{reviewComment}</p>
        </div>
      )}
        ...
        <div className="form-grid form-grid--three">
          ...
          <label className="field">
            <span>
              ...
              <strong style={{ color: "var(--color-error)" }}>*</strong>
            </span>
            <select
        ...
        <div className="form-grid form-grid--two">
          ...
          <div>
            ...
            <label className="check-field">
              ...
              />
              <span>
                ⚠️ Flag as the <strong>Key Issue</strong> for this week
              </span>
            </label>
          </div>
          ...
          <div>
            ...
            <label className="check-field">
              <input
                ...
                onChange={(event) =>
                  update("keyAchievement", event.target.checked)
                }
              />
              <span>
                🌟 Flag as the <strong>Key Achievement</strong> for this week
              </span>
            </label>
          </div>
        </div>
